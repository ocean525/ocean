 ...
 PROGRAM read_interpol_sfc
   ...
   !
   ! --- method 2: cressman weight
   real             :: cressman
   real, parameter  :: search_r = 1.0
   !
   ! 
   ! === Part 1: read 
   !
   ...
   CALL grads_plot
   !
   ! --- 2.5 interpolation: Method 2, cressman weight
   LOOP_cressman: &
   do i = 1, kx
   do j = 1, ky
     su10(i,j) = cressman( search_r, 'u10', slon(i), slat(j) )
     sv10(i,j) = cressman( search_r, 'v10', slon(i), slat(j) )
     sslp(i,j) = cressman( search_r, 'slp', slon(i), slat(j) )
   enddo
   enddo &
   LOOP_cressman
   !
 ...
 SUBROUTINE grads_plot
   ...
   kx = size(slon)
   ky = size(slat)
   open(1, file='sfc.dat', form='unformatted', access='direct', recl=kx*ky, action='write')
     write(1,rec=1) su10 
     write(1,rec=2) sv10 
     write(1,rec=3) sslp 
   close(1)
 ...
 FUNCTION cressman( radius, field, x, y ) result( res )
   !
   USE sfc_data, only: lon, lat, u10, v10, slp
   !
   implicit none
   !
   real, intent(in)             :: radius, x, y
   character(len=3), intent(in) :: field
   real :: res
   !
   real    :: total_weight, weight, dis, dx, dy, a
   integer :: i, j   
   !
   total_weight = 0.
   LOOP_cressman: &
   ...
   do j = 1, size(lat)
     ...
     if( dis < radius ) then
         weight = (radius*radius - dis*dis) / (radius*radius + dis*dis)
     else
         weight = 0.
     endif
     if( field == 'u10' ) then
   ...
   if( total_weight == 0. ) then
     a = -999.90  ! --- no bg values are available
   else
     a = a/total_weight
   endif
   res = a
